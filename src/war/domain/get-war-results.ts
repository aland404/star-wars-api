import type { WarEntity } from '../infrastructure/entities'
import { Faction } from '../../people/domain/people'
import type { WarResult } from './war'
import { getBattleResults } from './get-battle-results'

interface WarResults { winner: WarResult, loser: WarResult }

export function getWarResults(warEntity: WarEntity): WarResults | undefined {
  if (!warEntity.battles.length)
    return undefined
  ...
  warEntity.battles.forEach((battle) => {
    ...
    if (battleResults.winner.faction === Faction.REBELLION)
      rebellionNbOfVictories++
  })
